...
class Retriever:
    ...
    def retrieve_docs(self, query):
        ...
        top_indexes = np.argsort(scores)[::-1][:top_k_bm25]
        ...
        index.add_with_ids(
            filtered_embeddings,
            np.array(range(0, len(filtered_content))).astype(np.int64),
        )
        ...
        res_index = [
            filtered_content.index(filtered_content[_id])
