...
categories = soup.find_all(
    "div", class_="frame frame-default frame-type-uploads frame-layout-0"
)
categories.pop(0)
result_dict = {}
        ...
        for key in result_dict:
            ...
            try:
                for name in result_dict[key]:
                    ...
                    data = cur.fetchone()
                    cur.execute(
                        f'SELECT document_id FROM documents WHERE document_name ="{name}"  AND category_id={data[0]}'
                    )
                    ...
                    if len(doc_data) != 0:
                        ...
                        cur.execute(
                            "INSERT INTO documents (document_name,document_link,category_id) values (?,?,?)",
                            (name, result_dict[key][name], data[0]),
                        )
                        conn.commit()
            except:
